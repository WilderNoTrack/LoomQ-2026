...
前面几节讲的是「把门翻译对」。一个真正的转译器还有两件事要做，而且都直接影响真机结果。

### 比特路由（`loomq/passes/routing.py`）

真机不是全连通的。量旋 8 比特超导是一条**链**，3 比特核磁是**三角形**，
悟空是二维格点。`cx q[0], q[5]` 在链上根本没法直接执行——
必须先用 SWAP 把两个比特搬到相邻位置。

两家平台都会替你做这件事（量旋静默映射，本源看 `mappingFlag`），
**这正是这个缺口容易被忽略的原因**。

实现的是「简单且可验证」的算法而不是 SABRE：不做前瞻，沿最短路从**两端同时**
往中间走，代价 `⌊(d-1)/2⌋` 次 SWAP 而不是 `d-1` 次。
正确性用分布验证——19 个测试在线形/环形/网格/全连通四种拓扑上把路由后的电路
和原电路做**精确分布比对**，并断言每个两比特门都落在相邻比特上。

```
QFT-4 在 4 比特线形拓扑上：16 门 → 23 门（+7 SWAP），深度 10 → 16
                            分布保真度 1.000000000
```

`python3 -m loomq route circuits/ghz3.qasm --device superconductor_vp` 会报告
SWAP 数、深度变化、最终布局、电路图，以及与原电路的保真度。

### 电路优化（`loomq/passes/peephole.py`）

`optimize.py` 只处理显而易见的相邻情况。`peephole.py` 处理分解真正产生的两类模式：

- **自逆与互逆门对**，而且能**穿过与之对易的门**抵消。
  `h q[0]; x q[1]; h q[0];` 塌成 `x q[1]`——只看相邻是发现不了的。
  只使用无条件成立的对易规则（支撑集不相交、两个对角门）：
  漏掉一次抵消只损失一个门，判错一次损失的是答案。
- **单比特门串重合成**：连续的单比特门合成一个 2×2 酉矩阵，
  再用 ZYZ 展开成**至多三个旋转**。故意只处理**连续**的串——
  跨越其它操作重排需要穷尽的对易分析，而降级产生的本来就是连续串。

两种改写都精确到全局相位，测试用**完整态矢**比对（而不是 counts——counts 会掩盖相位错误）。

```
管线效果：QFT-4  16 → 14 门     Grover-3  21 → 17 门     两个相邻 Toffoli 直接归零
```

这在真机上是实打实的：悟空上那次 GHZ-3 有 10% 的 shots 落在主峰之外，
每少一个门就赎回一点。

## 9d. 稳定子模拟：Clifford 电路的多项式时间

态矢要 `2^n` 个振幅，所以内置模拟器卡在 26 比特。但 Gottesman-Knill 定理说：
只由 `h/s/cx` 这类门构成的电路，可以用 `n × 2n` 的二进制 tableau 跟踪——
`O(n²)` 空间、每个门 `O(n²)` 时间。

**这里不是教科书注脚，而是刚需**：GHZ 态是 Clifford 的，
而「给我一个 GHZ 态」是智能体最常收到的请求。用态矢，验证一个 30 比特 GHZ 是不可能的；
用 tableau，瞬间完成。

`loomq/sim/stabilizer.py` 实现的是 Aaronson-Gottesman CHP tableau，
含 destabilizer 行（让测量从 `O(n³)` 降到 `O(n²)`）。
非 Clifford 门（`t`、任意角 `rz`、`cu1`、`ccx`）**明确拒绝而不是近似**，
调用方回退到态矢。角度按数值判定，所以 `rz(pi/2)` 会被认出是 `s`。

引擎按电路自动选择，不需要配置：态矢能跑就用态矢（有闭式分布，可以无噪声分配 shots），
跑不动且是 Clifford 就用 tableau。

```
run(ghz(200), 'braket', 200)  →  200 比特，168 秒，只有全 0 和全 1 两种结果
run(ghz(100), 'braket', 200)  →  100 比特，22 秒
```

19 个测试：两个引擎都能跑的地方必须**分布一致**，
以及态矢**根本跑不动**的地方（120 比特 GHZ）仍要给出正确答案。

## 10. 已知边界

诚实列出来，比被发现好：

- 态矢模拟上限 26 比特（`loomq/sim/statevector.py::MAX_QUBITS`），纯 Python。
  超过这个规模只有 **Clifford** 电路能跑（走稳定子 tableau，已验证到 200 比特）；
  含 `t`、任意角 `rz`、`cu1` 或 `ccx` 的宽电路会明确报错而不是给近似答案。
  评测电路最多 5 比特，所以这个边界不影响评分。
- 路由用的是最短路 SWAP 插入，不做前瞻。SABRE 类算法在深电路上能少插 20-30% 的 SWAP；
  这里选了可验证性而不是最优性，因为正确性能被测试钉死而启发式的收益不能。
- 优化只做对易抵消和单比特串重合成，没有两比特块的 KAK 重合成，
  也没有针对具体硬件基础门集的重定向。
